...
async def upload_raster(file: UploadFile = File(...)):
    ...
    rid = uuid.uuid4().hex
    final = UPLOADS / f"{rid}{suffix}"
    tmp.rename(final)
    display_name = _unique_display_name(Path(file.filename).name, [it["name"] for it in db["items"]])
    entry = {"id": rid, "name": display_name, "path": str(final), "sha1": sha1}
    db["items"].append(entry)
    _save_db(db)
    # drop any old cached stats for safety
...
async def segment(
    ...
    fname = f"segment_{seg_id}.geojson"
    (SEGMENTS_DIR / fname).write_text(json.dumps(fc), encoding="utf-8")
    return _ok({"id": f"segment_{seg_id}", "geojson": fc, "geojson_url": f"/results/segments/{fname}"})
...
def get_segments_index():
    return _ok({"items": _segment_items()})
...
async def classify(
    segment_id: str = Form(...),
    method: str = Form("rf"),  # not used in simple variant
):
    seg_fp = SEGMENTS_DIR / f"{segment_id}.geojson"
    if not seg_fp.exists():
        alt = RESULTS / f"{segment_id}.geojson"
        if alt.exists():
            seg_fp = alt
        elif segment_id.endswith(".geojson"):
            seg_fp = SEGMENTS_DIR / segment_id
            if not seg_fp.exists():
                seg_fp = RESULTS / segment_id
            if not seg_fp.exists():
                return _bad("segment geojson not found", 404)
        else:
            return _bad("segment geojson not found", 404)

    samples_fp = RESULTS / f"samples_{segment_id}.json"
    if not samples_fp.exists():
        samples_fp = SAMPLES_DIR / f"{segment_id}.json"
    if not samples_fp.exists():
        return _bad("no samples saved for this segment", 400)

    fc = json.loads(seg_fp.read_text(encoding="utf-8"))
    samples = json.loads(samples_fp.read_text(encoding="utf-8")).get("samples", {})

    seg_to_class = {}
    for cls, ids in samples.items():
        for sid in ids:
            seg_to_class[str(sid)] = cls

    for feat in fc.get("features", []):
        props = feat.setdefault("properties", {})
        sid = props.get("segment_id")
        cls = seg_to_class.get(str(sid))
        if cls:
            props["class"] = cls

    out_id = uuid.uuid4().hex[:6]
    out_name = f"classify_{segment_id}_{out_id}.geojson"
    (CLASSIFY_DIR / out_name).write_text(json.dumps(fc), encoding="utf-8")
